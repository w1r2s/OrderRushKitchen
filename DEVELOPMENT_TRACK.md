...
Фокус:
- начать M4: перевести выдачу ингредиентов/напитков на UI-сетку выбора
- сохранить единый путь создания готовых позиций через `IServedMenuItemFactory`
- не менять order lifecycle из M2/M3, только слой выбора и выдачи

Первый практический срез:
- [ ] Определить контракт данных для UI-сетки выбора (ингредиенты/напитки)
- [ ] Подготовить `DrinkCounter` и `ContainerCounter` к открытию UI вместо мгновенной выдачи
- [ ] Зафиксировать точку подтверждения выбора и выдачи объекта игроку
