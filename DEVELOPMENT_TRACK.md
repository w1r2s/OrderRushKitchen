...
Задачи:
- [ ] Заменить текущее поведение `DeliveryService` на генерацию и сопровождение активных заказов.
- [ ] Заказы должны появляться при взаимодействии со стойкой принятия заказа, а не по глобальному таймеру.
- [ ] Каждый активный заказ должен иметь собственное время жизни или шкалу удовлетворённости.
- [ ] Добавить поддержку частичного выполнения заказа из нескольких позиций.
- [ ] Сохранить события успеха и провала для UI и аудио.
...
Задачи:
- [ ] Заменить текущее поведение `ContainerCounter` на открытие UI-сетки ингредиентов.
- [ ] Добавить стойку напитков с тем же паттерном взаимодействия.
- [ ] Вынести общую логику выбора в переиспользуемый UI/presenter слой.
- [ ] После подтверждения выбора выдавать игроку выбранный объект.
...
Фокус:
- начать M2 и перевести систему заказов на новую runtime-модель

Первый практический срез:
- [ ] Спроектировать интерфейс нового OrderService
- [ ] Ввести хранение списка ActiveOrder
- [ ] Добавить ручное создание нового заказа через метод сервиса
- [ ] Добавить Tick для активных заказов
- [ ] Определить правила удаления завершённых и проваленных заказов
