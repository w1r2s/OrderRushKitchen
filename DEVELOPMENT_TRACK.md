...
Фокус:
- начать M3: связать сдачу через `DeliveryCounter` с универсальным составом блюд (`MenuItemDefinitionSo.requiredIngredients`)
- убрать остатки legacy-типов из активного пути сдачи и тарелки
- подготовить перенос на новую модель проверки заказа без burger-специфики

Первый практический срез:
- [ ] Перевести проверку состава тарелки на полностью универсальный matcher без legacy dish-рецептов
- [ ] Подготовить `DeliveryCounter`/`PlateKitchenObject` к multi-item delivery path
- [ ] Зафиксировать контракт сдачи для будущего UI заказов (M5) без изменений логики уровня
